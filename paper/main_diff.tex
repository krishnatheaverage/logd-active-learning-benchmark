...
%DIF ADD main.tex            Tue Jul 14 12:18:19 2026
...
\newcommand{\selCorr}{0.63} %DIF > 
\newcommand{\selOverlap}{4} %DIF > 
\newcommand{\selNpareto}{4} %DIF > 
\newcommand{\selNlig}{63} %DIF > 
\newcommand{\selMax}{6.1} %DIF > 
 %DIF > 
 %DIF > 
%DIF -------
...
}\DIFaddend Calibration is thus essential when the uncertainty is reported \DIFdelbegin \DIFdel{, used for decision-making, or used to define an applicability domain or trigger experiments\mbox{%DIFAUXCMD
...
to }\DIFdelend \DIFaddbegin \DIFadd{or acted
upon\mbox{%DIFAUXCMD
\cite{Janet_2019,Yang_2023}}\hskip0pt%DIFAUXCMD
, and, for acquisition, only non-monotone calibration
matters, a distinction easy to }\DIFaddend conflate.

\begin{figure}[htbp]\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.5\linewidth]{../figures/calibration.pdf}
%DIFDELCMD < \caption{%%%
\DIFdelFL{Uncertainty calibration. }\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.8\linewidth]{../figures/calibration_subgroups.pdf}
\caption{\DIFaddFL{Subgroup coverage of nominal-$80\%$ intervals under a ligand-disjoint split.
...
\midrule
\DIFaddendFL Raw \DIFdelbeginFL \DIFdelFL{ensemble uncertainty is miscalibrated
}\DIFdelendFL \DIFaddbeginFL & \DIFaddFL{0.72 }& \DIFaddFL{-- }& \DIFaddFL{-- }& \DIFaddFL{-- }\\
...
\textbf{\DIFadd{A high $\log D$ is not sufficient: selectivity
(Figure~\ref{fig:sel}).}} \DIFadd{In separations chemistry the objective is not a large
distribution coefficient per se but }\emph{\DIFadd{discrimination}} \DIFadd{between lanthanides. For each
...
\caption{\DIFaddFL{Extraction strength }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{calibration error $0.103$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{max $\log D$}\DIFaddendFL ) \DIFdelbeginFL \DIFdelFL{; split-conformal calibration attains nominal coverage
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{versus selectivity }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{error $0.013$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{log separation factor,
max$-$min $\log D$ across lanthanides}\DIFaddendFL ) \DIFaddbeginFL \DIFaddFL{for each ligand}\DIFaddendFL . \DIFaddbeginFL \DIFaddFL{The two are only moderately
correlated, so a high $\log D$ does not imply a selective extractant; diamonds mark the
Pareto-optimal ligands.}\DIFaddendFL }
\DIFdelbeginFL %DIFDELCMD < \label{fig:cal}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:sel}
...
the uncertainty is reported or acted upon, \DIFdelbegin \DIFdel{but not merely to improve acquisition.
}\DIFdelend \DIFaddbegin \DIFadd{and use group-conditional calibration when
...
}\DIFaddend validation covers four ligands. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{We characterise selectivity (the spread of $\log D$
across lanthanides) and show it diverges from raw $\log D$, but a full multi-objective,
selectivity-aware acquisition loop is left as an extension; the }\DIFaddend conclusions concern
$\log D$ regression and may differ for classification \DIFdelbegin \DIFdel{or multi-objective
selectivity }\DIFdelend targets.
